...
log_CogTask <- as.data.frame(matrix(NA,3,6))
colnames(log_CogTask) <- c("Type","All","Calc","Reas","Spat","None")
log_CogTask$Type <- c("AllSubj","Dan","Kyu")
...
for(i in 1:8){
  ...
  rst_log <- multinom(CogTask~., data = trainData)
  # Overall
  ...
  y_train_prob_tree <- predict(rst_log,Calc_train,type="prob")[,2]
  y_train_hat <- predict(rst_log,Calc_train,type="class")
  y_test_prob_tree <- predict(rst_log,Calc_test ,type="prob")[,2]
  y_test_hat <- predict(rst_log,Calc_test,type="class")
  ...
  y_train_prob_tree <- predict(rst_log,Reas_train,type="prob")[,2]
  y_train_hat <- predict(rst_log,Reas_train,type="class")
  y_test_prob_tree <- predict(rst_log,Reas_test,type="prob")[,2]
  y_test_hat <- predict(rst_log,Reas_test,type="class")
  ...
  y_train_prob_tree <- predict(rst_log,Spat_train,type="prob")[,2]
  y_train_hat <- predict(rst_log,Spat_train,type="class")
  y_test_prob_tree <- predict(rst_log,Spat_test,type="prob")[,2]
  y_test_hat <- predict(rst_log,Spat_test,type="class")
  ...
  y_train_prob_tree <- predict(rst_log,None_train,type="prob")[,2]
  y_train_hat <- predict(rst_log,None_train,type="class")
  y_test_prob_tree <- predict(rst_log,None_test,type="prob")[,2]
  y_test_hat <- predict(rst_log,None_test,type="class")
...
# 5 column : all, Calc, Reas, Spat, None
log_CogTask[1,2] <- mean(rst$test[,1])
log_CogTask[1,3] <- mean(rst$test[,2])
log_CogTask[1,4] <- mean(rst$test[,3])
log_CogTask[1,5] <- mean(rst$test[,4])
log_CogTask[1,6] <- mean(rst$test[,5])
...
#   rst_log <- randomForest(CogTask~., data = trainData,
#                               mtry = danmtry , 
#                               ntree = overallntree, 
...
# 
# # 5 column : all, Calc, Reas, Spat, None
# log_CogTask[2,2] <- mean(rst$test[,1])
# log_CogTask[2,3] <- mean(rst$test[,2])
# log_CogTask[2,4] <- mean(rst$test[,3])
# log_CogTask[2,5] <- mean(rst$test[,4])
# log_CogTask[2,6] <- mean(rst$test[,5])
# 
# ################ Kyu ################ 
# set.seed(1)
# # equally sampling
...
# 
# # 5 column : all, Calc, Reas, Spat, None
# log_CogTask[3,2] <- mean(rst$test[,1])
# log_CogTask[3,3] <- mean(rst$test[,2])
# log_CogTask[3,4] <- mean(rst$test[,3])
# log_CogTask[3,5] <- mean(rst$test[,4])
# log_CogTask[3,6] <- mean(rst$test[,5])

log_CogTask
saveRDS(log_CogTask,file = "Output/CogTask_bACCbRT_log_Rst.Rdata")
# head(dta_4c_rst)
